...
for(const item of must){
  const [n,re]=item;
  if(!re.test(html+js))throw new Error('QA failed: '+n);
}
...
if(/正常播放 loops|normal play loops/i.test(fs.readFileSync('README.md','utf8')))throw new Error('QA failed: stale README');
